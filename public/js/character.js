...
.steward-wrap {
  ...
  --patch-o:0; --stitch-o:0; --cuff-fray:0; --btn2-o:1;
  --pocketsq-o:0; --coinstack-o:0;
  --band-progress:0;
  --band-lift:calc(var(--band-progress) * -1.6px);
  --band-straighten:calc(var(--band-progress) * -1.2deg);
...
.sw-upper-stack{left:0;right:0;top:0;height:176px;transform-origin:50% 100%;transform:translateY(calc(var(--upper-drop) + var(--band-lift))) rotate(calc(var(--chest-tilt) + var(--band-straighten)));z-index:4;overflow:visible;}
...
.sw-btn:nth-of-type(1){top:18px;}.sw-btn:nth-of-type(2){top:30px;opacity:var(--btn2-o);}.sw-btn:nth-of-type(3){top:42px;}
...
/* ── Suit condition arc: patched/frayed at the bottom, tailored at the top.
   Same silhouette every tier — only the repair state changes. ─────── */
.sw-coat-r::after{content:"";position:absolute;left:7px;top:40px;width:12px;height:11px;opacity:var(--patch-o);background:linear-gradient(135deg,#43332a,#2e211b);border:1px dashed rgba(228,209,180,0.6);border-radius:2px;transform:rotate(-7deg);}
.sw-coat-l::after{content:"";position:absolute;right:4px;top:9px;width:0;height:30px;opacity:var(--stitch-o);border-right:2px dashed rgba(230,214,188,0.62);transform:rotate(4deg);}
.sw-cuff::after{content:"";position:absolute;left:1px;right:1px;bottom:-2.5px;height:0;opacity:var(--cuff-fray);border-bottom:2px dashed rgba(222,214,200,0.85);}
.sw-body::after{content:"";position:absolute;top:29px;right:12px;width:12px;height:7px;opacity:var(--pocketsq-o);background:linear-gradient(180deg,#fffdf6,#e6ddcc);clip-path:polygon(0 100%,18% 0,46% 72%,72% 8%,100% 100%);filter:drop-shadow(0 1px 1px rgba(0,0,0,0.18));z-index:3;}
...
.sw-held-coin::before{content:"";position:absolute;left:50%;top:13px;width:19px;height:5px;margin-left:-9.5px;border-radius:999px;opacity:var(--coinstack-o);background:linear-gradient(180deg,#f2cf7b,#c8963a);box-shadow:0 5px 0 -0.5px #b8842c,0 10px 0 -1px #9a6818,inset 0 1px 0 rgba(255,248,220,0.55);}
...
.steward-money{inset:0;opacity:calc(var(--money-opacity) * (1 + var(--band-progress) * 0.45));transform:scale(var(--money-scale));z-index:2;}
...
.steward-wrap[data-state="rock_bottom"] .sw-mouth{height:6px;border-bottom-width:2px;border-radius:3px;background:repeating-linear-gradient(90deg,rgba(242,236,224,0.92) 0 2.5px,rgba(95,49,42,0.9) 2.5px 4.5px);}
...
.wealthy-mansion{display:none;opacity:0;pointer-events:none;transition:opacity 0.3s ease;}
...
.steward-wrap[data-state="wealthy"] .sw-vest{animation:vestGlow 2.0s ease-in-out infinite;overflow:hidden;}
@keyframes swVestSheenSweep{0%,58%,100%{transform:translateX(-14px) skewX(-16deg);opacity:0;}66%{opacity:0.55;}82%{transform:translateX(32px) skewX(-16deg);opacity:0;}}
.steward-wrap[data-state="wealthy"] .sw-vest::after{content:"";position:absolute;top:-2px;bottom:-2px;left:0;width:9px;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.5),transparent);animation:swVestSheenSweep 4.4s ease-in-out infinite;}
...
@keyframes swBreatheEase{0%,100%{transform:translateY(calc(var(--upper-drop) + var(--band-lift))) rotate(calc(var(--chest-tilt) + var(--band-straighten)));}50%{transform:translateY(calc(var(--upper-drop) + var(--band-lift) - 1.4px)) rotate(calc(var(--chest-tilt) + var(--band-straighten) - 0.4deg));}}
...
@keyframes swTierUpHatTip{0%,100%{transform:translateX(-50%) rotate(var(--hat-tilt));}30%,60%{transform:translateX(-54%) translateY(-6px) rotate(calc(var(--hat-tilt) - 14deg));}}
.steward-wrap.steward-tier-up .sw-hat{animation:swTierUpHatTip 0.9s cubic-bezier(0.34,1.56,0.64,1);}
...
/* ── Within-stage progress: someone 90% through a stage should read visibly
   closer to breaking through than someone who just entered it. One var
   (--band-progress, 0..1) drives a slight posture straighten and a brighter
   money field via CSS calc — no per-state map changes needed. ── */
function applyBandProgress(mount, bandPct) {
  const wrap = mount.firstElementChild;
  if (!wrap) return;
  const n = Number(bandPct);
  const t = Number.isFinite(n) ? Math.min(Math.max(n / 100, 0), 1) : 0;
  wrap.style.setProperty('--band-progress', t.toFixed(3));
}
...
export function mountHeroCharacter(stateId, opts = {}) {
  ...
  // Streak wear and band progress still update: both move without the tier changing.
  if (prev === stateId && mount.firstElementChild) {
    applyStreakWear(mount, opts.streakCurrent);
    applyBandProgress(mount, opts.bandPct);
    return;
  }
