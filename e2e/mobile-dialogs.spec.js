...
test('classify dialog is fully reachable and completable on mobile', async ({ page }) => {
  ...
  // The card caps to the visible viewport and scrolls INSIDE itself (the
  // overlay also scrolls as a backstop) — so the title starts on-screen and
  // everything below is reachable by scrolling the card.
  ...
  expect(probe.cardTop).toBeGreaterThanOrEqual(0);
  expect(probe.cardBottom).toBeLessThanOrEqual(probe.innerHeight);
  expect(probe.cardOverflowY).toBe('auto');
  expect(probe.cardScrollable).toBe(true);

  // Scroll the card to the bottom: Save must land inside the viewport.
  await page.evaluate(() => {
    const card = document.querySelector('#paydown-confirm-dialog .paydown-confirm-card');
    card.scrollTop = card.scrollHeight;
  });
  await page.waitForTimeout(200);
  const after = await page.evaluate(() => {
    const card = document.querySelector('#paydown-confirm-dialog .paydown-confirm-card');
    const save = card.querySelector('.paydown-confirm-save').getBoundingClientRect();
    return { scrollTop: card.scrollTop, saveTop: save.top, saveBottom: save.bottom, innerHeight: window.innerHeight };
  });
